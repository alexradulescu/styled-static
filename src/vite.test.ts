...
describe('plugin configuration', () => {
  ...
  it('should enforce pre order to run before other plugins', () => {
    const plugin = styledStatic();
...
const Link = styled.a\`color: blue;\`;`;
    ...
    const cssImportCount = countMatches(result?.code ?? '', /import "styled-static:/g);
...
const _PrivateCard = styled.div\`padding: 1rem;\`;`;
  ...
  it('should NOT transform let declarations (only const)', async () => {
    const code = `import { styled } from 'styled-static';
let Button = styled.button\`padding: 1rem;\`;`;
  ...
  it('should NOT transform var declarations (only const)', async () => {
    const code = `import { styled } from 'styled-static';
