...
function HowItWorksWrapper({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={clsx("opacity-100 transition-opacity duration-[350ms] ease-out", className)}
      style={{ "--how-it-works-section-loaded": 1 } as React.CSSProperties}
      {...props}
    />
  );
}
...
function LegendParagraph({ className, ...props }: React.ComponentProps<"p">) {
  return (
    <p
      className={clsx("mb-4 mt-0 text-sm text-[var(--color-text-secondary)]", className)}
      {...props}
    />
  );
}

function CompareTableWrap({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={clsx(
