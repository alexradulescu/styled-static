...
const Button = styledVariants({
  ...
  variants: {
    ...
    size: { sm: css`font-size: 0.875rem;`, lg: css`font-size: 1.125rem;` },
  },
  defaultVariants: { color: "primary", size: "sm" },
  compoundVariants: [{ color: "danger", size: "lg", css: css`font-weight: 900;` }],
});
...
const buttonClass = cssVariants({
  ...
  defaultVariants: { color: "primary", size: "sm" },
  compoundVariants: [{ color: "danger", size: "lg", css: css`font-weight: bold;` }],
});
...
`;
const Box = styled.div`background: var(--bg); color: var(--text);`;
```
...
const Button = styled.button`background: var(--btn-bg, gray);`;
...
```tsx
styled.div`color: ${someVar};`   // WRONG — build-time only
styled.div`color: var(--color);` // RIGHT
```
...
```ts
export default defineConfig({ css: { transformer: "lightningcss" }, plugins: [styledStatic(), react()] });
```
